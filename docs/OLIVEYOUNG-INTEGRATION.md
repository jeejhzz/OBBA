...
- `assets/js/data/catalog.js`에 상품 21종을 손으로 넣어뒀다. 이 중 **11종은 `goodsNo`를 확보**해
  상세페이지로 직행하고, 나머지 10종은 검색 결과로 폴백한다. 확보 현황과 잔여 목록은
  [GOODSNO-REVIEW.md](GOODSNO-REVIEW.md).
...
> **해보고 알게 된 것**: `goodsNo`는 전부 채우는 게 정답이 아니다.
> 올리브영은 같은 제품을 단품·기획·리필·콜라보로 여러 SKU로 파는데, 기획 SKU는 시즌마다 교체된다.
> 기획 번호를 박아두면 언젠가 판매종료 페이지로 간다.
> **안정적인 단품 SKU가 있는 상품만 채우고, 기획만 있는 상품은 검색 폴백을 유지하는 것**이 정책이다.
> 틀린 딥링크는 검색 폴백보다 나쁘다 — 사용자가 되돌아올 방법이 없기 때문.
