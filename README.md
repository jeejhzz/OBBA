...
- 자연어 인식은 형태소 분석 없이 **가중치 키워드 매칭**이다. 사전에 있는 말만 알아듣는다.
  못 알아듣는 표현을 발견하면 `tools/check-nlu.js` 에 한 줄 추가하고 `flows.js` 사전을 보강하는 게 정해진 절차다.
  진짜 언어 이해가 필요해지면 LLM 레이어(로드맵 P2)로 가야 한다.
